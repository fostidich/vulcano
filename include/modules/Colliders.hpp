...
inline void Collider::getModelExtentsAABB(Model *M, AABBextents &E) {
    ...
    if (!VD->Position.hasIt) {
        logs::error("Error! Vertex descriptor does not have position. Cannot create Bounding Box");
        return;
    }
    ...
    logs::debug(stride, " ", offset, " ", M->vertices.size(), " ", (M->vertices.size() / stride));
    for (int startPos = 0; startPos < M->vertices.size(); startPos += stride) {
        ...
        logs::debug(pos->x, ", ", pos->y, ", ", pos->z);
    }
    logs::debug(E.xMin, " ", E.yMin, " ", E.zMin, "    ", E.xMax, " ", E.yMax, " ", E.zMax);
}
...
inline bool Collider::collidesWith(Collider &dest) {
    ...
    logs::warning("Collision not supported between types");
    return false;
}
...
inline AABBextents Collider::getExtents() {
    ...
    default:
        logs::warning("getExtent not supported by this collider type");
        break;
    }
...
class ColliderShow {
  ...
  public:
    ...
    void init(BaseProject *_BP, int MaxColliders, int so = 11000) {
        ...
        VD.init(BP, {{0, sizeof(ColliderShowVertex), VK_VERTEX_INPUT_RATE_VERTEX}}, {{0, 0, VK_FORMAT_R32G32B32_SFLOAT, offsetof(ColliderShowVertex, pos), sizeof(glm::vec3), POSITION}});
    ...
    static void populateCommandBufferAccess(VkCommandBuffer commandBuffer, int currentImage, void *Params) {
        logs::debug("Populating access (", commandBuffer, ") for image: ", currentImage);
        ColliderShow *T = ((ColliderShowAndModel *)Params)->cls;
        T->populateCommandBuffer(commandBuffer, currentImage);
    }

    void populateCommandBuffer(VkCommandBuffer commandBuffer, int currentImage) {
        logs::debug("Populating for image: ", currentImage);
    ...
    void updateCommandBuffer() {
        ...
        if (commandBufferMustUpdate) {
            ...
            logs::debug("Submitting command buffer");
