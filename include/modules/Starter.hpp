...
struct Pipeline {
	...
	void setScissor(std::vector<VkRect2D> _scissor);
	void setViewport(std::vector<VkViewport> _viewport);
...
void PrintVkError( VkResult result ) {
	...
	logs::error("Error: ", result, ", ", meaning);
}

std::vector<char> readFile(const std::string& filename) {
	std::ifstream file(filename, std::ios::ate | std::ios::binary);
	if (!file.is_open()) {
		logs::error("Failed to open: ", filename);
		throw std::runtime_error("failed to open file!");
	}

	size_t fileSize = (size_t) file.tellg();
	std::vector<char> buffer(fileSize);
	logs::debug(filename, " -> ", fileSize, " B");
	file.seekg(0);
...
void BaseProject::createInstance() {
	logs::debug("Starting createInstance()");
...
VKAPI_ATTR VkBool32 VKAPI_CALL BaseProject::debugCallback(
	...
	logs::debug("Validation layer: ", pCallbackData->pMessage);
	return VK_FALSE;
}
...
void BaseProject::deviceReport::print() {
	logs::debug("swapChainAdequate: ", swapChainAdequate);
	logs::debug("swapChainFormatSupport: ", swapChainFormatSupport);
	logs::debug("swapChainPresentModeSupport: ", swapChainPresentModeSupport);
	logs::debug("completeQueueFamily: ", completeQueueFamily);
	logs::debug("anisotropySupport: ", anisotropySupport);
	logs::debug("extensionsSupported: ", extensionsSupported);

	for (const auto& ext : requiredExtensions) {
		logs::warning("Extension <", ext, "> unsupported.");
	}
}
...
void BaseProject::pickPhysicalDevice() {
	...
	logs::debug("Physical devices found: ", deviceCount);
		...
		if(includeRaytracing) {
			for(auto e : ExtensionsForRaytracing) {
				if(checkIfItHasDeviceExtension(device, e)) {
					...
					logs::debug("Raytracing extension found: ", e);
				} else {
					logs::warning("Raytracing extension missing: ", e);
					raytraceOk = false;
				}
			}
...
*/		}
		if(!raytraceOk) {
			logs::error("Error! Not all required extensions are available. Raytracing cannot be done.");
			exit(0);
		}
		...
		if (suitable) {
			...
			logs::debug("Maximum samples for anti-aliasing: ", msaaSamples);
			break;
		} else {
			logs::warning("Device ", device, " is not suitable");
			devRep.print();
		}
	}
...
void BaseProject::submitCommandBuffer(std::string name, int order, pNCBfunc populateNewCommandBuffer, void *params, pNCBfree onErase) {
	...
	if(found != namedCommandBuffers.end()) {
		...
		logs::debug("Existing command buffer '", name, "', old size: ", found->second.old.size());
	} else {
		// otherwise, add a new named command buffer
		namedCommandBuffers[name] = {nncb, {}};
		logs::debug("New command buffer '", name, "'");
	}
}
...
void BaseProject::removeBuffer(std::string name) {
	...
	if(found != namedCommandBuffers.end()) {
		...
		logs::warning("Try to delete a non-submitted command buffer: ", name);
	}
}
...
void BaseProject::clearNamedCommandBuffer(NamedCommandBuffer *ncb) {
	...
	if(ncb->cleaner != nullptr) {
		logs::debug("Calling cleaner function");
		ncb->cleaner(ncb->params);
	} else {
		logs::debug("Calling cleaner function is null, so no call for c.b. '", ncb->name, "'");
	}
	delete ncb;
}
...
void BaseProject::createCommandBuffer(NamedCommandBuffer *ncb, int imageIndex) {
	logs::debug("Buffer: '", ncb->name, "', id: ", imageIndex);
	...
	logs::debug("Finishing");
	if (vkEndCommandBuffer(*cb) != VK_SUCCESS) {
		throw std::runtime_error("failed to record command buffer!");
	}

	logs::debug("Closing: ", *cb);
...
void BaseProject::updateCommandBuffers(std::vector<VkCommandBuffer> &buffers, int imageIndex) {
	...
	for(auto &v : namedCommandBuffers) {
		logs::debug("Considering buffer: ", v.first);
...
//			buffers.push_back(*ncb->cb[imageIndex]);
			...
			logs::error("Error! state ", ncb->state, " not permitted here!");
		}

		std::vector<int> toDelete = {};
		for(int j = 0; j < v.second.old.size(); j++) {
			NamedCommandBuffer *ocb = v.second.old[j];
			logs::debug("Found old version for c.b. '", ocb->name, "'");
		...
		logs::debug(toDelete.size(), " old c.b. of '", v.first, "' can be erased");
...
void BaseProject::printFloat(const char *Name, float v) {
	logs::debug("float ", Name, " = ", v, ";");
}

void BaseProject::printVec2(const char *Name, glm::vec2 v) {
	logs::debug("glm::vec3 ", Name, " = glm::vec3(", v[0], ", ", v[1], ");");
}

void BaseProject::printVec3(const char *Name, glm::vec3 v) {
	logs::debug("glm::vec3 ", Name, " = glm::vec3(", v[0], ", ", v[1], ", ", v[2], ");");
}

void BaseProject::printVec4(const char *Name, glm::vec4 v) {
	logs::debug("glm::vec4 ", Name, " = glm::vec4(", v[0], ", ", v[1], ", ", v[2], ", ", v[3], ");");
}

void BaseProject::printMat3(const char *Name, glm::mat3 v) {
	logs::debug("glm::mat3 ", Name, " = glm::mat3(");
	for(int i = 0; i<9; i++) {
		logs::debug(v[i/3][i%3], ((i<8) ? ", " : ");"));
	}
}

void BaseProject::printMat4(const char *Name, glm::mat4 v) {
	logs::debug("glm::mat4 ", Name, " = glm::mat4(");
	for(int i = 0; i<16; i++) {
		logs::debug(v[i/4][i%4], ((i<15) ? ", " : ");"));
	}
}

void BaseProject::printQuat(const char *Name, glm::quat q) {
	logs::debug("glm::vec3 ", Name, " = glm::vec3(", q[0], ", ", q[1], ", ", q[2], ", ", q[3], ");");
}
...
void BaseProject::saveScreenshot(const char *filename, int currentBuffer) {
	...
	if (!(formatProps.optimalTilingFeatures & VK_FORMAT_FEATURE_BLIT_SRC_BIT)) {
		logs::warning("Device does not support blitting from optimal tiled images, using copy instead of blit!");
		supportsBlit = false;
	}
	...
	if (!(formatProps.linearTilingFeatures & VK_FORMAT_FEATURE_BLIT_DST_BIT)) {
		logs::warning("Device does not support blitting to linear tiled images, using copy instead of blit!");
		supportsBlit = false;
	}
	...
	logs::debug("Screenshot saved to disk");
			  ...
			  case VertexDescriptorElementUsage::POSITION:
			    if(E[i].format == VK_FORMAT_R32G32B32_SFLOAT) {
				  if(E[i].size == sizeof(glm::vec3)) {
					...
					logs::error("Vertex Position - wrong size");
				  }
				} else {
				  logs::error("Vertex Position - wrong format");
				}
			    break;
			  case VertexDescriptorElementUsage::POS2D:
			    if(E[i].format == VK_FORMAT_R32G32_SFLOAT) {
				  if(E[i].size == sizeof(glm::vec2)) {
					...
					logs::error("Vertex Position 2D - wrong size");
				  }
				} else {
				  logs::error("Vertex Position 2D - wrong format");
				}
			    break;
			  case VertexDescriptorElementUsage::NORMAL:
			    if(E[i].format == VK_FORMAT_R32G32B32_SFLOAT) {
				  if(E[i].size == sizeof(glm::vec3)) {
					...
					logs::error("Vertex Normal - wrong size");
				  }
				} else {
				  logs::error("Vertex Normal - wrong format");
				}
			    break;
			  case VertexDescriptorElementUsage::UV:
			    if(E[i].format == VK_FORMAT_R32G32_SFLOAT) {
				  if(E[i].size == sizeof(glm::vec2)) {
					...
					logs::error("Vertex UV - wrong size");
				  }
				} else {
				  logs::error("Vertex UV - wrong format");
				}
			    break;
			  case VertexDescriptorElementUsage::COLOR:
			    if(E[i].format == VK_FORMAT_R32G32B32_SFLOAT) {
				  if(E[i].size == sizeof(glm::vec3)) {
					...
					logs::error("Vertex Color - wrong size");
				  }
				} else {
				  logs::error("Vertex Color - wrong format");
				}
			    break;
			  case VertexDescriptorElementUsage::TANGENT:
			    if(E[i].format == VK_FORMAT_R32G32B32A32_SFLOAT) {
				  if(E[i].size == sizeof(glm::vec4)) {
					...
					logs::error("Vertex Tangent - wrong size");
				  }
				} else {
				  logs::error("Vertex Tangent - wrong format");
				}
			    break;
				case VertexDescriptorElementUsage::JOINTWEIGHT:
					if(E[i].format == VK_FORMAT_R32G32B32A32_SFLOAT) {
						if(E[i].size == sizeof(glm::vec4)) {
							...
							logs::error("Vertex Joint Weight - wrong size");
						}
					} else {
						logs::error("Vertex Joint Weight - wrong format");
					}
				break;
				case VertexDescriptorElementUsage::JOINTINDEX:
					if(E[i].format == VK_FORMAT_R32G32B32A32_UINT) {
						if(E[i].size == sizeof(glm::uvec4)) {
							...
							logs::error("Vertex Joint Index - wrong size");
						}
					} else {
						logs::error("Vertex Joint Index - wrong format");
					}
				break;
			  default:
			    break;
...
//	}
	...
	logs::debug("Loading Asset File: ", file, "[OBJ] - mat. path: ", (matpath == nullptr ? "<<NOPATH>>" : matpath));
...
void AssetFile::initGLTF(std::string file) {
	...
	logs::debug("Loading Asset File: ", file, "[GLTF]");
	...
	for (const auto& mesh :  model.meshes) {
		logs::debug(" Name:", mesh.name, " Primitives: ", mesh.primitives.size());
		int PrimCount = 0;
		for (const auto& primitive :  mesh.primitives) {
			if (primitive.indices < 0) {
				continue;
			} else {
				logs::debug("Primitive: ", PrimCount, ", Material: ",
...
void Model::loadModelOBJ(std::string file) {
	...
	logs::debug("[OBJ] Vertices: ", (vertices.size()/VD->Bindings[0].stride), " Indices: ", indices.size());
}
...
void Model::makeGLTFMesh(tinygltf::Model *M, const tinygltf::Primitive *Prm) {
	...
	if(pIt != Prm->attributes.end()) {
		...
		if(VD->Position.hasIt) {
			logs::warning("Warning: vertex layout has position, but file hasn't");
		}
	}
	...
	if(nIt != Prm->attributes.end()) {
		...
		if(VD->Normal.hasIt) {
			logs::warning("Warning: vertex layout has normal, but file hasn't");
		}
	}
	...
	if(tIt != Prm->attributes.end()) {
		...
		if(VD->Tangent.hasIt) {
			logs::warning("Warning: vertex layout has tangent, but file hasn't");
		}
	}
	...
	if(uIt != Prm->attributes.end()) {
		...
		if(VD->UV.hasIt) {
			logs::warning("Warning: vertex layout has UV, but file hasn't");
		}
	}
	...
	if(iIt != Prm->attributes.end()) {
		...
		if(VD->JointIndex.hasIt) {
			logs::warning("Warning: vertex layout has Joint, but file hasn't");
		}
	}
	...
	if(wIt != Prm->attributes.end()) {
		...
		if(VD->JointWeight.hasIt) {
			logs::warning("Warning: vertex layout has Weights, but file hasn't");
		}
	}
...
//std::cout << "\n";
	...
	switch(accessor.componentType) {
		...
		case TINYGLTF_PARAMETER_TYPE_UNSIGNED_INT:
			...
			break;
		default:
			logs::error("Index component type ", accessor.componentType, " not supported!");
...
void Model::loadModelGLTF(std::string file, bool encoded) {
	...
	logs::debug("Loading : ", file, (encoded ? "[MGCG]" : "[GLTF]"));
	if(encoded) {
...
//}
	...
	for (const auto& mesh :  model.meshes) {
		logs::debug("Primitives: ", mesh.primitives.size());
		for (const auto& primitive :  mesh.primitives) {
			if (primitive.indices < 0) {
				continue;
			}
	...
	logs::debug((encoded ? "[MGCG]" : "[GLTF]"), " Vertices: ", (vertices.size()/VD->Bindings[0].stride), " Indices: ", indices.size());
	logs::debug("Node[0] translation size: ", model.nodes[0].translation.size());
	logs::debug("Node[0] rotation size: ", model.nodes[0].rotation.size());
	logs::debug("Node[0] scale size: ", model.nodes[0].scale.size());
...
void Model::initMesh(BaseProject *bp, VertexDescriptor *vd, bool printDebug) {
	...
	int mainStride = VD->Bindings[0].stride;
	if(printDebug) {
		logs::debug("[Manual] Vertices: ", (vertices.size()/mainStride), " Indices: ", indices.size());
	}
	createVertexBuffer();
	createIndexBuffer();
	Wm = glm::mat4(1);
	  ...
	  case GLTF:
   	    {
   		  ...
   		  if(el != AF->GLTFmeshes.end()) {
   		  	...
   		  	if((Mid >= 0) && (Mid < P.size())) {
   		  		...
   		  		logs::error("Asset >", AN, "< does not have component: ", Mid);
   		  	}
   		  } else {
   		  	logs::error("Asset does not contain Mesh: ", AN);
   		  }
		  if(NN != "") {
			  ...
			  if(nel != AF->GLTFnodes.end()) {
				...
				logs::error("Asset does not contain Node: ", NN);
			  }
		  }
   	    }
		break;
	  case OBJ:
   	    {
   		  ...
   		  if(el != AF->OBJmeshes.end()) {
   		  	Prm = el->second;
   		  	if(Mid != 0) {
   		  		logs::error("OBJ assets can only be single material");
   		  	} else {
		...
		break;
	  default:
	    logs::error("Unknown asset file type: ", AF->type);
	    break;
	}
...
void Texture::createTextureImage(std::vector<std::string>files, VkFormat Fmt) {
	...
	for(int i = 0; i < imgs; i++) {
		...
		if (!pixels[i]) {
			logs::error("Not found: ", files[i]);
			throw std::runtime_error("failed to load texture image!");
		}
		logs::debug("[", i, "]", files[i], " -> size: ", texWidth, "x", texHeight, ", ch: ", texChannels);
...
void Texture::initCubic(BaseProject *bp, std::vector<std::string>files, VkFormat Fmt, bool initSampler) {
	if(files.size() != 6) {
		logs::error("Error! Cube map without 6 files - ", files.size());
		exit(0);
	}
	BP = bp;
	imgs = 6;
...
void Pipeline::init(BaseProject *bp, VertexDescriptor *vd,
	...
	logs::debug("Vertex shader <", VertShader, "> len: ", vertShaderCode.size());
	logs::debug("Fragment shader <", FragShader, "> len: ", fragShaderCode.size());
...
void Pipeline::setScissor(std::vector<VkRect2D> _scissor) {
 	scissor = _scissor;
	scissorSet = true;
}

void Pipeline::setViewport(std::vector<VkViewport> _viewport) {
 	viewport = _viewport;
	viewportSet = true;
}
...
void Pipeline::create(RenderPass *RP) {
	...
	if(!viewportSet) {
		...
		viewport[0].x = 0.0f;
		viewport[0].y = 0.0f;
		viewport[0].width = (float) RP->width;
		viewport[0].height = (float) RP->height;
	...
	if(!scissorSet) {
		...
		scissor[0].extent = {(uint32_t)RP->width, (uint32_t)RP->height};
	}
	...
	viewportState.pViewports = &viewport[0];
	viewportState.scissorCount = scissor.size();
	viewportState.pScissors = &scissor[0];
