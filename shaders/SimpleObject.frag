...
void main() {
    ...
    float shadow = computeShadow(fragPos, N, L, 0.2); // Compute shadow at fragment world coordinates
    vec3 directLight = computeDirectLight(N, L, H, albedo, gubo.lightColor.rgb, shadow); // Fragment direct color lights
    vec3 pointLight = computePointLights(fragPos, N, V, albedo, gubo.pointLightsCount, gubo.pointLights); // Compute all point lights contribution
