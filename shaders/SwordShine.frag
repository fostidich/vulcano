...
//Conductor metal has diffuse color equal to specular color instead of white (L09, mS = mD)
//Shininess exponent gamma is a spectrum (L09 slide 76: cos a, cos^2 a, cos^8 a, cos^64 a):
//low gamma = broad, always-visible sheen; high gamma = tight mirror spike.
//This mesh is low-poly with flat per-face normals (no smooth curvature to sweep through
//alignment), so a tight spike (gamma=200, then 24, then 2) never lands. Using gamma=1,
//the broadest point on that same spectrum, so the highlight is visible reliably.
const float baseSpecularStrenght = 0.005; //dull
const float shineSpecularStrenght = 8.0; //strong metallic response, compensates for the broad exponent
const float baseShininess = 10.0;
const float shineShininess = 1.0; //broadest point on L09's cos^gamma spectrum
...
void main() {
    ...
    float specularStrenght = mix(baseSpecularStrenght, shineSpecularStrenght, gubo.swordShine);
    float shininess = mix(baseShininess, shineShininess, gubo.swordShine);
    ...
    vec3 specular = albedo * specularStrenght * pow(HdotN, shininess); // mS = mD
    vec3 diffuse = albedo * NdotL; // Intensity of object reflection
    vec3 directLight = (diffuse + specular) * radiance; // Fragment direct color lights
    ...
    for (int i = 0; i < gubo.pointLightsCount; ++i) {
        ...
        vec3 pSpecular = albedo * specularStrenght * pow(pNdotH, shininess);
