...
void Vulcano::updateDeersBridge(float deltaT) {
    ...
    const float targetHeight = 7.5f;
    static float currentTime = 0.0f;                  // Progress is computed as: current time / duration
    ...
    // Manage bridge animation state for this frame
    if (world.deerBridgeRaised)
        currentTime -= deltaT;
    else
        currentTime += deltaT;
    if (currentTime <= 0.0f) {
        world.deerBridgeMoving = false;
        world.deerBridgeRaised = false;
    } else if (currentTime >= duration) {
        world.deerBridgeMoving = false;
        world.deerBridgeRaised = true;
    }
